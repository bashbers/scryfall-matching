...
function renderSwiper(
  card = sampleCard,
  queuedCards = [] as (typeof sampleCard)[],
) {
  const onLike = vi.fn();
...
test("shows commander status, tracks viewing, and flips double-faced cards", async () => {
  ...
  expect(screen.getByRole("img", { name: "Sample Card" })).toHaveAttribute(
    "src",
    sampleCard.frontImageUrl,
  );
...
test("uses a local back and keeps the other face usable after an image error", async () => {
  ...
  expect(
    screen.getByRole("img", { name: "Sample Card" }).getAttribute("src"),
  ).toContain("card-back");
  expect(onViewed).toHaveBeenCalledOnce();
});
